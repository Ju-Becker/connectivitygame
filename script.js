...
const hoverclick = new Audio('Sound/click-21156.mp3');
hoverclick.volume =0.3;
...
lastmoveBtn.addEventListener('click', () => {
  if (!lastmove_available || game_over) return;
  lastMove();
});
...
canvas.addEventListener('click', e => {
  if (game_over || botismoving) return;
  const rect = canvas.getBoundingClientRect();
  const mx = e.clientX - rect.left;
  const my = e.clientY - rect.top;
  takeEdge(mx, my);
...
function showResultBanner(playerName,didPlayerWin) {
  ...
  if(botDifficulty=="off"){
    if(playerName=="M"){
      ...
      const audio = new Audio('Sound/goodresult-82807.mp3');
      audio.volume = 0.6;
      audio.play().catch(e => {
      console.warn("Autoplay blockiert – der Benutzer muss zuerst interagieren.", e);
      });
    }else{
    ...
    if(playerName=="M"){
      ...
      //-----------------
      const audio = new Audio('Sound/booohwav-14768.mp3');
      audio.volume = 0.6;
      audio.play().catch(e => {
      console.warn("Autoplay blockiert – der Benutzer muss zuerst interagieren.", e);
      });
      ...
      banner.innerHTML = `🎉 Glückwunsch, <strong>Breaker</strong>! Du hast gewonnen! 🎉`;  
      //-----------------
      const audio = new Audio('Sound/dj-airhorn-sound-39405.mp3');
      audio.volume = 0.6;
      audio.play().catch(e => {
      console.warn("Autoplay blockiert – der Benutzer muss zuerst interagieren.", e);
      });
    ...
    if(playerName=="M"){
      banner.innerHTML = `😢 Der Computer hat gewonnen. Viel Glück beim nächsten Mal.`;
      //-----------------
      const audio = new Audio('Sound/failure-1-89170.mp3');
      audio.volume = 0.6;
      audio.play().catch(e => {
      console.warn("Autoplay blockiert – der Benutzer muss zuerst interagieren.", e);
      });
